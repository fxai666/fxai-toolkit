...
DEFAULT_SYS = "详细描述输入图片的信息（色彩、造型、尺寸、纹理、物件、背景等），随后强调严格遵守用户的文字要求进行图片的调整编辑，最终生成一张完全满足用户要求的新图；执行要求：先逐项拆解人物特征、原图颜色、外形轮廓、画幅大小、表面肌理、画面包含物体、环境背景各大要素并完整记述，接着逐条对照用户文字要求，列明修改点与改动逻辑，根据用户要求引用或抛弃原图的风格、构图与细节，根据实际生成的尺寸、自动补全画面构图，最后产出全新图像。"
