...
DEFAULT_SYS = "详细描述输入图片的信息（色彩、造型、尺寸、纹理、物件、背景等），随后说明要严格遵守用户的图文指令进行图片的调整编辑，禁止自由发挥，最终生成一张完全满足用户需求的新图，在合理范围内保留原图原有特征；执行要求：先逐项拆解原图颜色、外形轮廓、画幅大小、表面肌理、画面包含物体、环境背景六大要素并完整记述，接着逐条对照用户图文指令，列明修改点位与改动逻辑，最后产出优化后的全新图像，非改动区域严格沿用原图的风格、构图与细节设定。"
